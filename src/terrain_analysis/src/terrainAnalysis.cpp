...
int main(int argc, char** argv)
{
  ...
  auto subOdometry =
    nh->create_subscription<nav_msgs::msg::Odometry>("state_estimation", 5, odometryHandler);
  ...
  auto pubTotalLaserCloud = nh->create_publisher<sensor_msgs::msg::PointCloud2>("/total_terrain_map", 2);
  ...
  while (status) {
    ...
    if (newlaserCloud) {
      ...
      for (int i = 0; i < terrainCloudSize; i++) {
        ...
        int indX =
          int((point.x - vehicleX + planarVoxelSize / 2) / planarVoxelSize) + planarVoxelHalfWidth;
        int indY =
          int((point.y - vehicleY + planarVoxelSize / 2) / planarVoxelSize) + planarVoxelHalfWidth;
            ...
            if (planarVoxelEdge[i] >= 1) {
              ...
              for (int dX = -1; dX <= 1; dX++) {
                for (int dY = -1; dY <= 1; dY++) {
                  if (indX + dX >= 0 && indX + dX < planarVoxelWidth && indY + dY >= 0 &&
                      indY + dY < planarVoxelWidth) {
                    if (planarVoxelEdge[planarVoxelWidth * (indX + dX) + indY + dY] <
                        planarVoxelEdge[i]) {
                      edgeVoxel = true;
                    }
                  }
