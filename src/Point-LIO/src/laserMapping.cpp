...
int main(int argc, char** argv)
{
  ...
  auto pubLaserCloudFullRes =
    nh->create_publisher<sensor_msgs::msg::PointCloud2>("cloud_registered", 5);
  auto pubLaserCloudFullRes_body =
    nh->create_publisher<sensor_msgs::msg::PointCloud2>("cloud_registered_body", 5);
  ...
  while (rclcpp::ok()) {
    ...
    if (sync_packages(Measures)) {
      ...
      if (runtime_pos_log) {
        ...
        time_log_counter++;
        printf("[ mapping ]: time: IMU + Map + Input Downsample: %0.6f ave "
